...
# task3
## ResNe50 model [Training the core layers+Trainging only the top layers]
...
def plot_confusion_matrix(cm, model_name):
  df_cm = pd.DataFrame(
      cm,
      index=[artists_top_name.iloc[i].replace('_', ' ') for i in range(artists_top_name.shape[0])],
      columns=[artists_top_name.iloc[i].replace('_', ' ') for i in range(artists_top_name.shape[0])]
  )
